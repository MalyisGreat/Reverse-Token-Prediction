...
parser.add_argument("--reverse-training", action="store_true", help="reverse each packed row after BOS so causal training predicts previous tokens")
# Model architecture
...
if args.reverse_training:
    print0("Reverse training enabled: rows are BOS + reversed token body, so the causal target is the previous token.")
train_loader = tokenizing_distributed_data_loader_with_state_bos_bestfit(
    ...
    reverse_rows=args.reverse_training,
)
